...
void shader::usse::utils::buffer_address_load(spv::Builder &b, const SpirvShaderParameters &params, SpirvUtilFunctions &utils, const FeatureState &features, Operand &dest, spv::Id addr, uint32_t component_size, uint32_t nb_components, bool is_fragment, int buffer_idx) {
    ...
    spv::Id buffer_address = b.createAccessChain(spv::StorageClassUniform, params.render_info_id, { b.makeIntConstant(render_buffer_idx), buffer_idx_val });
    ...
    if (component_size == sizeof(uint32_t)) {
        ...
        if (nb_components >= 4) {
            ...
            while (nb_components >= 4) {
                spv::Id loaded = b.createAccessChain(spv::StorageClassPhysicalStorageBuffer, buffer_address_vec4, { zero, b.makeIntConstant(buffer_idx_vec4) });
        ...
        if (nb_components > 0) {
            ...
            spv::Id loaded = b.createAccessChain(spv::StorageClassPhysicalStorageBuffer, buffer_address_vec, { zero, b.makeIntConstant(buffer_idx_vec4) });
            loaded = b.createLoad(loaded, spv::NoPrecision, spv::MemoryAccessAlignedMask, spv::ScopeMax, 4);
        ...
        for (int component_idx = 0; component_idx < nb_components; component_idx++) {
            ...
            spv::Id loaded = b.createAccessChain(spv::StorageClassPhysicalStorageBuffer, component_addr, { zero, zero });
            loaded = b.createLoad(loaded, spv::NoPrecision, spv::MemoryAccessAlignedMask, spv::ScopeMax, 4);
